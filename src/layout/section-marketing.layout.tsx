...
export default function MarketingSection({
  ...
  icon = (
    <ShoppingBag className="w-16 h-16 mx-auto mb-6 text-primary-foreground" />
  ),
  features = [
    {
  ...
  return (
    <div
      className={cn("text-center space-y-8", className, " mx-auto w-full")}
      {...props}
    >
      <div>
        {icon}
        <h1 className="text-4xl font-bold mb-4">{title}</h1>
        <p className="text-xl opacity-90 max-w-md mx-auto">{subtitle}</p>
      </div>

      <div className="space-y-4 max-w-sm mx-auto flex flex-col items-center">
        {features.map((feature, index) => (
          <div key={index} className="flex items-center gap-3">
            <span className={cn(feature.iconColor)}>{feature.icon}</span>
